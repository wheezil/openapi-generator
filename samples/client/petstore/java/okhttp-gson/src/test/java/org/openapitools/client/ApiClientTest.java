...
public class ApiClientTest {
	...
	/*
	 * @Test public void testSetUsernameAndPassword() { HttpBasicAuth auth =
	 * null; for (Authentication _auth :
	 * apiClient.getAuthentications().values()) { if (_auth instanceof
	 * HttpBasicAuth) { auth = (HttpBasicAuth) _auth; break; } }
	 * auth.setUsername(null); auth.setPassword(null);
	 * 
	 * apiClient.setUsername("my-username");
	 * apiClient.setPassword("my-password"); assertEquals("my-username",
	 * auth.getUsername()); assertEquals("my-password", auth.getPassword());
	 * 
	 * // reset values auth.setUsername(null); auth.setPassword(null); }
	 */
	...
	public void testParameterToPairsWhenValueIsEmptyStrings() throws Exception {
		...
		List<Pair> concatStrings = apiClient.parameterToPairs("csv", "param-a",
				strs);

		assertEquals(1, concatStrings.size());
		assertFalse(concatStrings.get(0).getValue().isEmpty()); // should
																// contain some
																// delimiters
	}
	...
	public void testParameterToPairsWhenValueIsCollection() throws Exception {
		...
		collectionFormatMap.put("unknown", ","); // all other formats, must
													// default to csv
		...
		List<Pair> multiPairs = apiClient.parameterToPairs("multi", name,
				values);
		assertEquals(values.size(), multiPairs.size());
		for (int i = 0; i < values.size(); i++) {
			assertEquals(apiClient.escapeString(apiClient
					.parameterToString(values.get(i))), multiPairs.get(i)
					.getValue());
		}

		// all other formats
		for (String collectionFormat : collectionFormatMap.keySet()) {
			List<Pair> pairs = apiClient.parameterToPairs(collectionFormat,
					name, values);
			...
			if (!delimiter.equals(",")) {
				// commas are not escaped because they are reserved characters
				// in URIs
			...
			for (int i = 0; i < values.size(); i++) {
				assertEquals(apiClient.escapeString(apiClient
						.parameterToString(values.get(i))), pairValueSplit[i]);
			}
		}
	}
	...
	public void testSanitizeFilename() {
		...
		assertEquals("sun.gif",
				apiClient.sanitizeFilename("\\var\\tmp\\sun.gif"));
		assertEquals("sun.gif",
